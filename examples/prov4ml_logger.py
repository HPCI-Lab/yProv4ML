...
for epoch in tqdm(range(EPOCHS)):
    for i, (x, y) in enumerate(train_loader):
        ...
        logger.log(item=loss.item(), identifier="MSE_train", kind="metric", context="Training", step=epoch)
    ...
    logger.log(item=epoch, identifier="epoch", kind="carbon", context="Training", step=epoch)
    # prov4ml.log_system_metrics(prov4ml.Context.TRAINING, step=epoch)
    logger.log(item=epoch, identifier="epoch", kind="system", context="Training", step=epoch)
    ...
    logger.log(item=mnist_model, identifier=f"mnist_model_version", kind="model", context="Training", step=epoch)
...
for i, (x, y) in tqdm(enumerate(test_loader)):
    ...
    logger.log(item=loss.item(), identifier="MSE_test", kind="metric", context="Test", step=epoch)
